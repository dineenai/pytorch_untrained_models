...
# parser.add_argument('--mmk', default=False, type=bool, #metavar='N',
#                     help='Implement blur as multiple-mini kernels') #Added
parser.add_argument('--mmk',  dest='mmk', action='store_true', #metavar='N',
                    help='Implement blur as multiple-mini kernels') #Added
...
#     valdir = os.path.join(args.data, 'val') # Need to change Val to val in folders as this is the directory name on server
    ...
    print(f'Confirm Parameters are set correctly:\n\targs.gauss: {args.gauss}, args.mmk: {args.mmk}, args.kernel: {args.kernel}')
   
    # Blur functions for MMK
    if args.mmk:
        print(f'value of mmk: {args.mmk}')
        if args.gauss==4:
            gauss = 2
            blur_kernel = 13
            ...
            # MMK not yet impleemented for provided args.gauss
            raise ValueError(f'MMK blur transform not yet implemented for provided sigma: {args.gauss}')
    ...
    elif args.mmk: #Added to implement multiple mini kernels
        train_dataset = datasets.ImageFolder(
            traindir,
            transforms.Compose([
                transforms.RandomResizedCrop(224),
