...
parser.add_argument('--lr', '--change-lr', default=0.1, type=float,
                    metavar='LR', help='initial learning rate', dest='lr')
parser.add_argument('--change_lr', '--learning-rate', default=30, type=int,
                    metavar='CHANGELR', help='change epoch every n epochs', dest='change_lr')
...
#     valdir = os.path.join(args.data, 'val') # Need to change Val to val in folders as this is the directory name on server
    ...
    # Working train_dataset
    # train_dataset = datasets.ImageFolder(
    #     traindir,
    #     transforms.Compose([
    #         transforms.RandomResizedCrop(224),
    #         transforms.GaussianBlur(9, sigma=(args.gauss, args.gauss)),
    #         transforms.RandomHorizontalFlip(),
    #         transforms.ToTensor(),
    #         normalize,
    #     ]))
...
# Args is not defined - why is his inuse in main_general? INVESTIGATE THIS! - added globally defined args - should work!
...
def adjust_learning_rate(optimizer, epoch, args):
    # original regime - used by defauult unless otherwise specified
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    # lr = args.lr * (0.1 ** (epoch // 30))

    # """Sets the learning rate to the initial LR divided by sqrt(10) every 15 epochs"""
    # # Lies on the same function as above, just sampled twice as often.
    # # lr = args.lr * (0.1 / math.sqrt(10) ** (epoch // 15))
    ...
    # lr = args.lr  / (math.sqrt(10) ** (epoch // 15))
    # lr = round(lr, 8)

    # see plot_lr_trajectory.py to see how this trajectory looks
    """Sets the learning rate to the initial LR divided by sqrt(10) every change_lr epochs"""
    lr = args.lr * (0.1 ** (epoch // args.change_lr))
