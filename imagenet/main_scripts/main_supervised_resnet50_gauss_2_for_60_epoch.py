...
#     valdir = os.path.join(args.data, 'val') # Need to change Val to val in folders as this is the directory name on server
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose([
            ...
            transforms.GaussianBlur(9, sigma=(2, 2)),
        ...
        # save running checkpoint From Anna T DeepCluster https://github.com/AnnaTruzzi/deepcluster/blob/backtostart/main.py
            torch.save({'epoch': epoch + 1,
                     ...
                     #  os.path.join(args.exp, 'checkpoint_dc'+str(args.instantiation)+'_epoch'+str(epoch)+'.pth.tar'))
                       os.path.join(args.model_path, 'checkpoint_supervised_resnet50_gauss_2_for_60'+'_epoch'+str(save_epoch)+'.pth.tar'))
...
def save_checkpoint(state, is_best, filename='checkpoint_supervised_resnet50_gauss_2_for_60.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_supervised_resnet50_gauss_2_for_60.pth.tar')
