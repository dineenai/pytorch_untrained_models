# Preferable to subselect desired subcategories - animals, inanimate snmall and inanimate large in jupyter notebbok
...
def validate(val_loader, model, criterion, args):
    ...
    # animal_counter = 0
    # inanimate_counter = 0
    # inanimate_small_counter = 0
    # inanimate_large_counter = 0
    ...
    # inanimate_small_hape = 0
    # inanimate_small_tex = 0
    # inanimate_small_other = 0

    # inanimate_large_shape = 0
    # inanimate_large_tex = 0
    # inanimate_large_other = 0
    ...
    # Can I jsut select desired elements from list above??? ############
    # if "a" == ["a", "b", "c"]:
    
    # for the animal labels 

    # sub select only the components desired!!!

    # for i in len animal
    # if animali = any element of 

    # animal_tex = [0] * len(categories_long)
    # animal_shape = [0] * len(categories_long)
    # animal_other = [0] * len(categories_long)
    # animal_shape_bias = [0] * len(categories_long)


    save_shape_bias_df = '/home/ainedineen/blurry_vision/pytorch_untrained_models/imagenet/shape_bias_df/'+str(args.save_accuracy_file)+'.txt'
    f_df = open(save_shape_bias_df, 'a')
    # Add contents to f_df in line with geirhos's df
    # Add via loop below
    # Can I make this a svv from the get go?
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            for i in range(0, len(categories_short)):
                if (categories_short[i] == short_shape):
                    category = str(categories_long[i])
            ...
            f_df.write('{0},'
                ...
                '{4},'
                '\n'.format(
                    str(args.save_accuracy_file), counter, decision_from_16_classes,
                    category, short_sample_fname))  
        ...
        # for i in range(0, len(categories_short)):
        #     # if categories_short[i] == animal:
        #     for categories_short[i] == 'dog' || 'cat' || 'bea'|| 'bir'|| 'ele':
        #         animal_shape = animal_shape + categories_shape[i]
        #         animal_tex = animal_tex + categories_tex[i]
        #         animal_other = animal_other + categories_other[i]

        #     print(animal_shape)
        #     print(animal_tex)
        #     print(animal_other)

        #     # if categories_short[i] == inanimate:
        #     for categories_short[i] == ['air', 'bic', 'boa', 'car', 'cha', 'key', 'ove', 'bot',  'clo', 'kni', 'tru']:
        #         # inanimate_shape = inanimate_shape + categories_shape[i]
        #         inanimate_shape+=categories_shape[i]
        #         inanimate_tex = inanimate_tex + categories_tex[i]
        #         inanimate_other = inanimate_other + categories_other[i]

        #     print(inanimate_shape)
        #     print(inanimate_tex)
        #     print(inanimate_other)

        #     # if categories_short[i] == inanimate_large:
        #     if categories_short[i] == ['air', 'bic', 'boa', 'car', 'cha',  'key', 'ove', 'tru']:
        #         inanimate_large_shape = inanimate_large_shape + categories_shape[i]
        #         inanimate_large_tex = inanimate_large_tex + categories_tex[i]
        #         inanimate_large_other = inanimate_large_other + categories_other[i]
            
        #     # if categories_short[i] == inanimate_small:
        #     if categories_short[i] == ['clo', 'bot', 'kni']:
        #         inanimate_small_shape = inanimate_small_shape + categories_shape[i]
        #         inanimate_small_tex = inanimate_small_tex + categories_tex[i]
        #         inanimate_small_other = inanimate_small_other + categories_other[i]

        #     # did these need to be initialised to 0???
        # animal_shape_bias = (animal_shape/(animal_shape + animal_tex))
        # inanimate_shape_bias = (inanimate_shape/(inanimate_shape + inanimate_tex))
        # inanimate_small_shape_bias = (inanimate_small_shape/(inanimate_small_shape + inanimate_small_tex))
        # inanimate_lareg_shape_bias = (inanimate_large_shape/(inanimate_large_shape + inanimate_large_tex))

        # print(animal_shape_bias)
        # print(inanimate_shape_bias)
        # print(inanimate_small_shape_bias)
        # print(inanimate_lareg_shape_bias)

        # # If this works THEN CREATE A DF
