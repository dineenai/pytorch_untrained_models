...
def validate(val_loader, model, criterion, args):
    ...
    #  Added Texture Bias
    categories_tex_tex = [0] * len(categories_long)
    categories_shape_tex = [0] * len(categories_long)
    categories_other_tex = [0] * len(categories_long)
    categories_texture_bias = [0] * len(categories_long)
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            for i in range(0, len(categories_short)):
                ...
                # Added: Texture Bias
                elif (short_tex == categories_short[i]): #FIXED?
                    print(categories_long[i])
                    if (short_shape == short_tex):
                        print("same")
                    elif(short_shape == short_decision):
                        categories_shape_tex[i]+=1
                    elif(short_tex == short_decision):
                        categories_tex_tex[i]+=1
                    else:
                        categories_other_tex[i]+=1

        # Globally shape and texture bias are the inverse of each other, this is not true at the category level?
        print("Shape Bias: ")
        shape_bias = (shape_counter/(shape_counter + tex_counter))
        print(shape_bias)

        print("Texture Bias: ")
        texture_bias = (tex_counter/(shape_counter + tex_counter))
        print(texture_bias)        
        ...
        # Added texture bias
        for i in range(0, len(categories_short)):
            categories_texture_bias[i] = (categories_tex_tex[i])/(categories_shape_tex[i] + categories_tex_tex[i])
        ...
        save_texture_bias = '/home/ainedineen/blurry_vision/pytorch_untrained_models/imagenet/shape_texture_bias/shape_texture_bias_'+str(args.save_accuracy_file)+'_'+str(args.test_result_stimuli_name)+'.txt'
        f3 = open(save_texture_bias, 'a')
        f3.write('Category, '
                    'Shape_Choices, '
                    'Texture_Choices, '
                    'Other_Choices, '
        ...
        for i in range(0, len(categories_short)):
            f3.write('{0}, '
                ...
                '{4}, '
                '\n'.format(
                    categories_long[i], categories_shape_tex[i], categories_tex_tex[i],
                    categories_other_tex[i], categories_texture_bias[i]))   
        f3.close
