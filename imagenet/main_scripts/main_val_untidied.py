...
#         os.makedirs(opt.model_folder)
    ...
    #Added Code - from CMC LinearProbing_val.py - is there a better way?
    #Assuming that order of files is preserved
    global listOfFiles
    listOfFiles = getListOfFiles(valdir)

    for i in range(len(listOfFiles)):
        listOfFiles[i] = os.path.basename(listOfFiles[i])

    # From: https://stackoverflow.com/questions/56962318/printing-image-paths-from-the-dataloader-in-pytorch
    ...
    # val_dataset = ValDataset(ImageFolderDataset=folder_dataset_val,
    #                                         transform=transforms.Compose([transforms.Resize((100,100)),
    #                                                                     transforms.ToTensor()
    #                                                                     ])
    #                                     ,should_invert=False)
    # folder_dataset_val = ImageFolderWithPaths(valdir)
    ...
    # for paths in folder_dataset_val:
    # use the above variables freely
        # ntpath.basename(paths) # TypeError: expected str, bytes or os.PathLike object, not tuple
        # ntpath.basename(paths[1]) #TypeError: expected str, bytes or os.PathLike object, not int
        # ntpath.basename(paths[0]) #TypeError: expected str, bytes or os.PathLike object, not Image
        # global short_path
        # short_path = ntpath.basename(paths[2]) 
        # print(short_path)
    ...
    # val_loader = torch.utils.data.DataLoader(folder_dataset_val, transforms.Compose([
    #         transforms.Resize(256),
    #         transforms.CenterCrop(224),
    #         transforms.ToTensor(),
    #         normalize,
    ...
    # #Still need to encorporate the above into loader      
    # val_loader = torch.utils.data.DataLoader(
    #     folder_dataset_val, transforms.Compose([
    #         transforms.Resize(256),
    #         transforms.CenterCrop(224),
    #         transforms.ToTensor(),
    #         normalize,
    ...
    # #Try from: https://discuss.pytorch.org/t/pytorch-output-image-name/88299
    # filename=CellsDataset(args.valdir,transform=ToTensor(),return_filenames=True) #NameError: name 'CellsDataset' is not defined
    # print(filename)

    # print(val_loader) #<torch.utils.data.dataloader.DataLoader object at 0x7f9d57518cd0>
...
#                        top1=top1))
                ...
                #Indent Problem??????? NB add print actual identity of image = Target! for comparison ALSO add this all to txt file!
...
#                         '\n'.format(
#                          listOfFiles[i], i, len(val_loader), loss=losses,
#                          top1=top1, top5=top5))
#                          #listOfFiles[i], i, len(val_loader), loss=losses))
#                 # print(val_loader[i]) #Try
...
#                         'Output: {out}\t'
                ...
                # NB NEITHER listOfFiles[i], or short_path is working yet!!!
