	...
	methods: {
		...
		getUrlQuery: function getUrlQuery() {
			...
			Object.keys(this.$route.query).forEach(function (key) {
				...
				} else if (typeof _this2.filters[key] === 'boolean' && ['true', 'false'].includes(paramValue)) {
		...
		reloadSelfData: function reloadSelfData() {
			...
			try {
				var filters = JSON.parse(JSON.stringify(this._actualFilters));
				var result = this.loadSelfData(filters, ctx);
...
export { paginationMixin, remove, update, addOrUpdate, handleGraphqlRequest, toGqlArg, gqlTag, convertGraphql };
